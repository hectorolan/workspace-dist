...
<!-- workspace-local:start -->
## Workspace adaptation (node --test — read this first in THIS repo)

This workspace's suites run on the Node built-in runner (`npm test` →
`node --test "cli/test/*.test.js"`). Apply this skill's METHODOLOGY — edge-case
enumeration, error/rejection paths, malformed-response fixtures, table-driven
cases, dependency injection instead of module mocks — but write `node:test`
syntax. Never add Jest as a dependency or convert a suite (decided in
skills-integration Phase 1b; a stack change needs its own CEO-gated phase).

| Jest idiom | node:test equivalent here |
|---|---|
| `describe/it/test` | `import { test } from 'node:test'` (subtests via `t.test`) |
| `expect(x).toBe(y)` | `assert.equal(x, y)` (`node:assert/strict`) |
| `expect(x).toEqual(y)` | `assert.deepEqual(x, y)` |
| `expect(fn).toThrow(...)` | `assert.throws(fn, /msg/)` |
| `await expect(p).rejects...` | `await assert.rejects(p, /msg/)` |
| `expect(s).toMatch(/re/)` | `assert.match(s, /re/)` / `assert.doesNotMatch` |
| `jest.fn()` / `jest.mock()` | inject fakes through function parameters (see `sweepPRs({gh, query, log, plans})` in `cli/util/prwatch.js`) |
| `jest.spyOn` call capture | in-file fake stores that record writes (see `fakeStore`/`fakePlans` in `cli/test/prwatch.test.js`) |
| `test.each(table)` | plain `for (const [in, out] of table)` around asserts |
| `jest.useFakeTimers()` | `node:test` mock timers (`t.mock.timers`) — rarely needed here |
<!-- workspace-local:end -->
