// VENDORED from workspace cli/util-tools/case-id-check.js @ e5f29fd — source of
// truth lives there (it is dependency-free by design, which is what makes this
// copy possible). Vendored because this repo's CI runner has no workspace
// checkout and the workspace repo is private (a fetch would need a new PAT
// secret). When the workspace copy changes, re-copy it here verbatim below this
// header — never fork the logic. (.mjs, not .js: this repo has no
// `"type": "module"` and the tool is ESM.)
//
